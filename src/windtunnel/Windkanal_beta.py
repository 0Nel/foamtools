...
def plot_fig (alpha, Cd, Cl, Re):	# Cd_std, Cl, Cl_std):
    ...
    plt.plot(alpha, Cl, color = 'red', linestyle = 'dashed', label = legend)
    plt.grid()
    ...
    plt.plot([minAngle, maxAngle], [minCd, minCd], color = 'black', linewidth = 0.75)
    plt.plot(alpha, Cd, color = 'blue', linestyle = 'dashed', label = legend)
    plt.grid()
    plt.ylim(0, max(Cd))
    plt.xlabel("Anstellwinkel [" + deg + "]")
    plt.ylabel("$C_d$")
...
#    Cl_std = mVtoLift(conversion_lift, data[:,CL_STDV], Al, Uc)
    ...
    plot_fig(alpha, Cd, Cl, Re) # Cd_std, Cl, Cl_std)
